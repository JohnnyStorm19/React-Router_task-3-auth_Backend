...
const news = [
  {
    ...
    image: `https://loremflickr.com/640/480?random=${Math.random()}`,
  ...
  },
  {
    id: uuid.v4(),
    title: faker.lorem.words(),
    image: `https://loremflickr.com/640/480/dog?random=${Math.random()}`,
    content: faker.lorem.paragraph(),
  },
  {
    id: uuid.v4(),
...
app.get("/private/news/:id", async (req, res) => {
  ...
  const newsCardInfo = news.find(o => o.id === id);
  try {
    res.send(JSON.stringify(newsCardInfo));
  } catch (error) {
    res.status(500).send(JSON.stringify({ message: "Server internal error" }));
  }
});
